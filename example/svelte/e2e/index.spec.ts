...
import { changeFlagStatus, cleanup, seed } from '@progressively/playwright-helpers/cmd';
...
test.describe('/', () => {
	...
	test('shows the old variant when the flag is not activated', async ({ page }) => {
	...
	test('shows the new variant when the flag is activated for homepage', async ({ page }) => {
	...
	test('uses the same cookie for anonymous users after page reload', async ({ page, context }) => {
