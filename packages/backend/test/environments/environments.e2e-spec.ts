...
describe('Environments (e2e)', () => {
  ...
  describe('/environments/1/experiments (POST)', () => {
    it('gives a 401 when the user is not authenticated', () =>
      verifyAuthGuard(app, '/environments/1/experiments', 'post'));

    it('gives a 403 when trying to access an invalid environment', async () => {
      const access_token = await authenticate(app);

      return request(app.getHttpServer())
        .post('/environments/3/experiments')
        .set('Authorization', `Bearer ${access_token}`)
        .send({
          name: 'New flag',
          description: 'The new flag aims to xxx',
    ...
    it('gives a 403 when the user requests a forbidden environment', async () => {
      ...
      return request(app.getHttpServer())
        .post('/environments/1/experiments')
        .set('Authorization', `Bearer ${access_token}`)
        .expect(403)
        .expect({
          statusCode: 403,
